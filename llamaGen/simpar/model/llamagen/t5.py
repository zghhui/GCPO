...
class T5Embedder:
    ...
    def __init__(self, device, dir_or_name='t5-v1_1-xxl', *, local_cache=False, cache_dir=None, hf_token=None, use_text_preprocessing=True,
        ...
        self.model = T5EncoderModel.from_pretrained(path, **t5_model_kwargs).cuda().eval()
        # self.model = T5EncoderModel.from_pretrained(path).to(self.device).eval()
    ...
    def get_text_embeddings(self, texts):
        ...
        return text_tokens_and_mask['input_ids'], text_tokens_and_mask['attention_mask'], text_encoder_embs, text_tokens_and_mask['attention_mask'].to(self.device)

    def get_embeddings_from_ids(self, ids):
        with torch.no_grad():
            model_device = self.model.device
            text_encoder_embs = self.model(
                input_ids=ids.to(model_device),
            )['last_hidden_state'].detach()
            text_encoder_embs = text_encoder_embs.to(self.device)
