...
class Geneval:
    def __init__(self):
        self.url = "http://173.3.243.249:18085"
...
def aps_reward(completions, **kwargs):
...
geneval_loss = Geneval()
def geneval_reward(prompts, images, meta_data):
    rewards = geneval_loss(prompts, images, good_image=None, num_generations=None, geneval_meta_data=meta_data)
    return rewards
    
def hps_reward(completions, **kwargs):
    image_features = [completion[0]["hps_image_features"] for completion in completions]
    text_features = [completion[0]["hps_text_features"] for completion in completions]
