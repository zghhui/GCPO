...
class GRPOTrainer(Trainer):
    ...
    def select_high_entropy_gradient_tokens_batch(self, entropy_seq, topk_frac=0.1):
        ...
        # Step 1: 5邻域（含自身、左、左上、上、右上）均值
        mean_map = torch.zeros_like(entropy_map)
        for i in range(h):
            for j in range(w):
                neighbors = [entropy_map[:, i, j]]  # 自身
                if j > 0:
                    neighbors.append(entropy_map[:, i, j-1])      # 左
                if i > 0 and j > 0:
                    neighbors.append(entropy_map[:, i-1, j-1])   # 左上
                if i > 0:
                    neighbors.append(entropy_map[:, i-1, j])     # 上
                if i > 0 and j < w-1:
                    neighbors.append(entropy_map[:, i-1, j+1])   # 右上
        ...
        for b in range(B):
            grad_sub = grad_flat[b, offset:offset+N_valid]  # 中间80%
            k = int(N * topk_frac)
            k = min(k, grad_sub.shape[0])
            _, idx = torch.topk(grad_sub, k)
            selected_mask[b, offset + idx] = 1  # offset补回实际索引
